...
{
    uint32 i, min = 255, cn = 0;
    uint32 c, dl, im;
    int sc;
    c =    (img[yx+dr[0]] + img[yx+dr[1]] + img[yx+dr[2]] +
            img[yx+dr[3]] + img[yx+dr[4]] + img[yx+dr[5]] +
            img[yx+dr[6]] + img[yx+dr[7]] + img[yx])/9;
    sc = c - img[yx];
    ...
    if(sc > 0 ){
        for(i=0; i < 8; i++) {
            if(img[yx+dr[i]] > c) {
                cn++;
                dl = abs(c - img[yx+dr[i]]);
                if(dl < min) { min = dl; im = i; }
            }
        }
    } else {
        for(i=0; i < 8; i++) {
...
{
    uint32 y, x, yx, yw, w2 = w-2, h2 = h-2;
    int dr[8] = { -1, -1-w, -w, +1-w, 1, 1+w, w, -1+w };
    uint32 nd;

    img1[w + 1] = 255; for(x=2; x < w2; x++) img1[w + x] = 255; img1[w + x] = 255;
    ...
    for(y=2; y < h2; y++){
        ...
        for(x=2; x < w2; x++){
            yx = yw + x;
            nd = col_shift(img, yx, dr);
            if(nd < 8) img1[yx] |= (1<<nd);
            //else img1[yx] = 129;
        }
        img1[yx + 1] = 255;
    }
    yw = y*w;
    img1[yw + 1] = 255; for(x=2; x < w2; x++) img1[yw + x] = 255; img1[yw + x] = 255;

    printf("Finish seg_mean_shift\n");
}

static inline uint8 get_region_color_ms(uint8 *img, uint8 *con, uint8 *con1, uint32 *l1, int *dr, uint32 num, uint32 cl, uint32 w)
{
    uint32  c = 0, cn = 0, k = 0, yx, j;
    uint32 *l2 = &l1[num];
    uint8 col1 = 255;
    ...
    while(num){
        for(j=0; j < num; j++){
            //printf("x = %d y = %d\n", l1[j]%w, l1[j]/w);
            yx = l1[j] + dr[0];  if(!con[yx] || (con[yx] != col1 && con[yx]&16))    { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[1];  if(!con[yx] || (con[yx] != col1 && con[yx]&32))    { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[2];  if(!con[yx] || (con[yx] != col1 && con[yx]&64))    { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[3];  if(!con[yx] || (con[yx] != col1 && con[yx]&128))   { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[4];  if(!con[yx] || (con[yx] != col1 && con[yx]&1))     { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[5];  if(!con[yx] || (con[yx] != col1 && con[yx]&2))     { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[6];  if(!con[yx] || (con[yx] != col1 && con[yx]&4))     { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            yx = l1[j] + dr[7];  if(!con[yx] || (con[yx] != col1 && con[yx]&8))     { con[yx] = col1; con1[yx] = cl; l2[k++] = yx; }
            c += img[l1[j]]; cn++;
        }
        num = k; k = 0;
        l1 = l2; l2 = &l1[num];
...
static inline void fill_region_color_ms(uint8 *con, uint32 *l1, uint32 num, uint8 col, uint32 w)
{
    uint32 k = 0, yx, j;
    uint32 *l2 = &l1[num];
...
{
    uint32 y, x, yx, yw, w2 = w-2, h2 = h-2, rgc = 0;
    int dr[8] = { -1, -1-w, -w, +1-w, 1, 1+w, w, -1+w };
    uint8 col = 129, cl = 0;
    ...
    for(y=2; y < h2; y++){
        ...
        for(x=2; x < w2; x++){
            ...
            if(!con[yx]){
                ...
                cl = get_region_color_ms(img, con, con1, buff, dr, 1, cl, w);
                //printf("%d color = %d\n", rgc, cl);
                rgc++;
            }
        }
    }
    printf("seg_mean_shift_color: number of regions = %d\n", rgc);
}
...
static inline uint32 check_max1(uint8 *img, int *dr, uint32 yx, uint8 val)
{
...
static inline uint32 check_diagonal(uint8 *img, int *dr, uint32 yx, uint8 di)
{
    if      (di == 1 && img[yx+dr[0]] > 252 && img[yx+dr[2]] > 252) return 1;
    else if (di == 3 && img[yx+dr[2]] > 252 && img[yx+dr[4]] > 252) return 1;
    else if (di == 5 && img[yx+dr[4]] > 252 && img[yx+dr[6]] > 252) return 1;
    else if (di == 7 && img[yx+dr[6]] > 252 && img[yx+dr[0]] > 252) return 1;
    return 0;
}
static inline uint32 check_diagonal1(uint8 *img, int *dr, uint32 yx, uint8 di)
{
    if      (di == 1 && img[yx+dr[0]]&8   && img[yx+dr[2]]&128) return 1;
    else if (di == 3 && img[yx+dr[2]]&32  && img[yx+dr[4]]&2  ) return 1;
    else if (di == 5 && img[yx+dr[4]]&128 && img[yx+dr[6]]&8  ) return 1;
    else if (di == 7 && img[yx+dr[6]]&2   && img[yx+dr[0]]&32 ) return 1;
    return 0;
}
...
/*	\brief	Find the maximum around the pixel.
    ...
    \retval			The direction of of local max.
*/
static inline int dir3(uint8 *con, uint8 *grad, int *dr, uint32 yx, int in1, uint32 w)
{
    uint8 max = 0,  i;
    int in = 0, tmp[3];
    /*
    ...
    if(con[yx+in]){
        if(check_diagonal1(con, dr, yx, find_dir(dr, in)) ){
            ...
            //print_con_grad(grad, con, yx, w);
            //printf("First ");
        } else if(check_max1(grad, dr, yx+in, 255) ){
            if(in == tmp[0]) tmp[0] = tmp[2];
            else if(in == tmp[1]) tmp[1] = tmp[2];
        } else return in;
    ...
    if(!check_diagonal1(con, dr, yx, find_dir(dr, in))) {
        if(con[yx+in]){
            ...
            if(check_max1(grad, dr, yx+in, 255)){
...
{
    uint32 i = 0;
    for(i=0; i < 8; i++) if(img[yx+dr[i]] < img[yx]) return 0;
    return img[yx];
}
*/
...
static inline uint32 draw_line_1(uint8 *img, Vector *v, uint32 w, uint8 col)
{
    ...
    for(i=0; i < dma; i++){
        ...
        if(i > 0 && i < dma-1 && img[yx] == 200) {
            printf("draw_line_1: The line is ander virtex x = %d y = %d\n", yx%w, yx/w);
            img[yx] = 255;
            //img[yx+1] = 255;
            //img[yx-1] = 255;
            //img[yx+w] = 255;
...
#ifdef DEBUG
        ////Check if vertex is in another line
        if(i > 0 && i < dma-1 && img[yx] == 200) printf("draw_line_2: The line is ander virtex x = %d y = %d\n", yx%w, yx/w);
#endif
        if(v->x2 > v->x1) {
            if(i == 1) xy[0] = yx;
            if(i == dma-2) xy[1] = yx;
...
static inline uint32 check_line3(Line_buff *buf, uint32 first, uint32 last, uint32 *lf, uint32 *rt, uint32 w)
{
    ...
    max = dma; l = dma-2;
    //if(x2 <= x1 && (dma&1 || dmi&1)) min = dmi-1;
    //Check the first+1 and last-1 pixels if they have the same direction with curve
    *lf = 0; *rt = 0;
    ...
    if(dx >= dy){
        if(x2 > x1){
            for(i=0; i < dma; i++){
                ic = i+first;
                //if(i == 1 || i == l) buf[ic].yx = x + y*w;
                //buf[ic].yx = x + y*w;
                if(x == buf[ic].x && y == buf[ic].y) {
                    n++;
                    if(i == 1) *lf = 1;
                    if(i == l) *rt = 1;
                }
            ...
            //If differnt line and curve lenght should be only if line = 3 pixel and curve = 4 pixel
            //if(dma -1 != last - first) buf[ic].yx = buf[ic-1].yx;
...
static inline uint32 check_line2(Line_buff *buf, uint32 first, uint32 last, uint32 w)
{
    uint32 i, ic, max , min = 0, n = 1, l, x, y, x1, x2, y1, y2, dx, dy;
    int  sty;
    int dma, dmi, stmi, stma;

    //x = x1; y = y1;
    x1 = buf[first].x<<1; y1 = buf[first].y<<1;
    x2 = buf[last ].x<<1; y2 = buf[last ].y<<1;
    //Check if line
    ...
    if(dx >= dy){
        if(x2 > x1){
            ...
            for(i=1; i < dma; i++){
                ...
                min += dmi; x += stmi;
                if(min >= max) { max += dma; y += stma; }
                if(i&1) { if(x == (buf[ic-1].x + buf[ic].x) && y == (buf[ic-1].y + buf[ic].y)) n++; }
                else    { if(x == (buf[ic].x<<1) && y == (buf[ic].y<<1)) n++; }
                //printf("x = %d buf = %d buf1 = %d y = %d buf = %d buf1 = %d ic = %d i = %d n = %d\n",
                //      x, buf[ic].x<<1, buf[ic-1].x + buf[ic].x, y, buf[ic].y<<1, buf[ic-1].y + buf[ic].y, ic, i, n);
            }
            //printf("\n");
            //If differnt line and curve lenght should be only if line = 3 pixel and curve = 4 pixel
            //if(dma -1 != last - first) buf[ic].yx = buf[ic-1].yx;
    ...
    //printf("dma = %d n = %d x = %d y = %d x1 = %d y1 = %d\n", dma, n, buf[first].x, buf[first].y, buf[last].x, buf[last].y);
    return n*100/dma;
}
...
static inline uint32 get_first_pixel2(uint8 *img, uint32 yx, uint32 yx1, uint32 yx2, int *dr)
{
    uint32 i, nd1, nd2, ndt;
    if(yx1 == yx2) return 0;

    if      (yx1-yx == dr[0]) nd1 = 0;
    else if (yx1-yx == dr[1]) nd1 = 1;
    else if (yx1-yx == dr[2]) nd1 = 2;
    else if (yx1-yx == dr[3]) nd1 = 3;
    else if (yx1-yx == dr[4]) nd1 = 4;
    else if (yx1-yx == dr[5]) nd1 = 5;
    else if (yx1-yx == dr[6]) nd1 = 6;
    else if (yx1-yx == dr[7]) nd1 = 7;

    if      (yx2-yx == dr[0]) nd2 = 0;
    else if (yx2-yx == dr[1]) nd2 = 1;
    else if (yx2-yx == dr[2]) nd2 = 2;
    else if (yx2-yx == dr[3]) nd2 = 3;
    else if (yx2-yx == dr[4]) nd2 = 4;
    else if (yx2-yx == dr[5]) nd2 = 5;
    else if (yx2-yx == dr[6]) nd2 = 6;
    else if (yx2-yx == dr[7]) nd2 = 7;

    ndt = nd1;
    for(i=0; ndt != nd2; i++) ndt = (ndt == 7) ? 0 : ndt + 1;
    i--;
    //printf("nd1 = %d nd2 = %d i = %d\n", nd1, nd2, i);

    if(i && i < 4){
        i = (nd1 == 7) ? 0 : nd1 + 1;
        while(i != nd2){
            if(!img[yx+dr[i]]) return yx+dr[i];
            i = (i == 7) ? 0 : i + 1;
            //printf("i = %d nd2 = %d img[yx+dr[i]] = %d\n", i, nd2, img[yx+dr[i]]);
        }
    } else return 0;
    return 0;
}
...
static inline uint32 get_first_pixel1(uint8 *img, Vertex *v, uint8 nd1, uint8 nd2, uint32 w)
{
    ...
    //i = get_n_round(yx, v->yx[nd1], v->yx[nd2], w);
    ...
    if(i && i < 4){
        ...
        do{
            yxn = get_next_pixel(yx, yxn, w);
            printf("x = %d y = %d x1 = %d y1 = %d img[yxn] = %d\n", yx%w, yx/w, yxn%w, yxn/w, img[yxn]);
            //if(yxn != yx2 && !img[yxn] && check_true_reg(img, yx, yxn, w)) return yxn;
                    ...
                    while(1){
                        ...
                        if(con[yx1]) {
                            ...
                            if(check_vertex(grad, yx1, 255, w)){
                                ...
                                //}
                                //if(yxl == yxt && yx1 > 252) yxc++;

                                grad[yx1] = 0;
                                yx1 = yxt; d1 = d3; //yxl = yxt;
                                goto m1;
                            }
                            if(d1) add_dir3(con, yx1, -d1, w);
                            grad[yx1] = 255; //con[yx1] = 255;
...
void seg_find_intersect7(uint8 *grad, uint8 *con, uint32 *hist, uint32 lmaxc, uint32 w, uint32 h)
{
    uint32 y, y1, x, yx, yw, yx1, yx2, yx3, yxt, i, h2 = h-2, w2 = w-2, max;//, yxl = 0;
    int d1, d2, d3, npix = 0, cr;
    int cn, ck = 0;
    ...
    for(i=0; i < lmaxc; i++){
        if(!con[hist[i]]){
            ...
            d2 = d1;
            //x = 1551 y = 810 x = 1077 y = 1174
            //if(yx1 == 1077 + 1174*w){
            //    printf("d1 = %d d2 = %d con[yx1+d1] = %d con[yx1+d2] = %d\n", d1, d2, con[yx1+d1], con[yx1+d2]);
            //}
            //printf("seg_find_intersect7: x = %d y = %d d1 = %d d2 = %d\n", yx1%w, yx1/w, d1, d2);
            //d3 = -d1;
            //if(d1 && d2 && !con[yx1+d1] && !con[yx1+d2]){
            if(!con[yx1+d1]){
                grad[yx1] = 253;
                ...
                while(!cr && cn < 2){
                    while(1){
                        //yxt = yx1;
                        //printf("seg_find_intersect7: x = %d y = %d d1 = %d\n", yx1%w, yx1/w, d1);
                        /*
                        if(yx1 == 1407 +(80-2)*w || yx1 == 1407+1 +(80-2)*w || yx1 == 1407 +(80-3)*w ) {
                            printf("seg_find_intersect7: x = %d y = %d d1 = %d\n", yx1%w, yx1/w, d1);
                            print_around(con, yx1, w);
                            print_around(grad, yx1, w);
...
//Find next pixel following clockwise direction
{
    ...
    if(*nd < 8){
        while(1){
            *nd = ((*nd) == 7) ? 0 : (*nd) + 1;
            if(con[yx + dr[*nd]] ) return yx + dr[*nd];
        }
    } else {
        nd1 = 0;
        while(1){
            *nd = (nd1 == 7) ? 0 : nd1 + 1;
            if(!con[yx + dr[nd1]] && con[yx + dr[*nd]]) return yx + dr[*nd];
            nd1 = *nd;
        }
    }
}
...
void seg_find_intersect8(uint8 *img, uint8 *grad, uint8 *con, uint32 *l1, uint32 *br, uint32 w, uint32 h)
{
                ...
                while(num){
                    for(j=0; j < num; j++){
                        ...
                        for(i=0; i < 4; i++){
                            yx1 = l1[j] + dr[i<<1];
                            //yx1 = l1[j] + dr[i];
                            //printf("x = %d y = %d grad = %d con = %d k = %d\n", yx1%w, yx1/w, grad[yx1], con[yx1], k);printf("x = %d y = %d grad = %d con = %d k = %d\n", yx1%w, yx1/w, grad[yx1], con[yx1], k);
                            if(grad[yx1] >= grad[l1[j]]) {
                                ...
                                //printf("x = %d y = %d grad = %d con = %d k = %d\n", yx1%w, yx1/w, grad[yx1], con[yx1], k);
                            } else if(grad[yx1] < grad[l1[j]] && !con[yx1]) tmp++;
                            //} else if(grad[yx1] <= grad[l1[j]] && con[yx1] != col && con[yx1]){
                               //con[yx1] = 255;
                            //}
                        }
                        if(tmp) br[ip++] = l1[j];
    ...
    printf("Finesh seg_find_intersect8\n");
...
static inline void finish_two_dirs(Vertex *vx1, Vertex *vx2, Line_buff *buf1, Line_buff *buf2)
{
    uint8 n1, n2;
    int dx, dy;
    ...
    if(check_same_vertex(vx1, vx2, n1)){
        //Remove collapsed to the line direction
        //printf("finish_two_dirs : Same vertex n1 = %d\n", n1);
        //print_pointer(vx1);
        //print_pointer(vx2);
        vx1->n--; vx2->n--;
...
/*
static inline void finish_two_dirs1(Vertex *vx1, Vertex *vx2, Line_buff *buf1, Line_buff *buf2, Vector *v)
{
    uint8 n;
    int dx, dy;
    //New algorithm
    ...
    dx = buf1->x - vx1->x; dy = buf1->y - vx1->y;
    if      (dx == -1 && dy ==  0) n = 0;
    else if (dx == -1 && dy == -1) n = 1;
    else if (dx ==  0 && dy == -1) n = 2;
    else if (dx ==  1 && dy == -1) n = 3;
    else if (dx ==  1 && dy ==  0) n = 4;
    else if (dx ==  1 && dy ==  1) n = 5;
    else if (dx ==  0 && dy ==  1) n = 6;
    else if (dx == -1 && dy ==  1) n = 7;

    if(x1 != buf1->x || y1 != buf1->y){
        vx1->di ^= (1<<n);
        dx = x1 - vx1->x; dy = y1 - vx1->y;
        if      (dx == -1 && dy ==  0) n = 0;
        else if (dx == -1 && dy == -1) n = 1;
        else if (dx ==  0 && dy == -1) n = 2;
        else if (dx ==  1 && dy == -1) n = 3;
...
static inline uint32 finish_two_dirs1(Vertex *vx1, Vertex *vx2, uint32 yx1, uint32 yx2, int *dr, uint32 w)
{
...
#ifdef DEBUG
    if(n1 == 8 || n2 == 8) printf("!!!!!!!!!!Problem n1 = %d n2 = %d\n", n1, n2);
#endif
    ...
    if(vx1->di&(1<<n1)) {
        ...
        if(vx1->x == 525 && vx1->y == 657){
                printf("finish_two_dirs1: %p n1 = %d x1 = %d y1 = %d  vx1->vp[n1]->x = %d vx1->vp[n1]->y = %d vx2->x = %d vx2->y = %d di = %d cn = %d\n",
                       vx1->vp[n1], n1, vx1->x, vx1->y, vx1->vp[n1]->x, vx1->vp[n1]->y, vx2->x, vx2->y, vx1->di, vx1->cn);
                print_pointer(vx1);
                //printf("nt = %d vx1->di&(1<<nt) = %d\n", (n1 == 7) ? 0 : n1 + 1, vx1->di&(1<<((n1 == 7) ? 0 : n1 + 1)));
        }

        nt = n1;
        nt = (nt == 7) ? 0 : nt + 1;
        if(vx1->x == 525 && vx1->y == 657) printf("nt = %d vx1->di&(1<<nt) = %d\n", nt, vx1->di&(1<<nt));
        //printf("nt = %d vx1->di&(1<<nt) = %d\n", nt, vx1->di&(1<<nt));
...
#ifdef DEBUG
            if(vx1->di&(1<<nt)) {
                printf("finish_two_dirs1: Problem %p n1 = %d x1 = %d y1 = %d  vx1->vp[n1]->x = %d vx1->vp[n1]->y = %d vx2->x = %d vx2->y = %d di = %d cn = %d\n",
                       vx1->vp[n1], n1, vx1->x, vx1->y, vx1->vp[n1]->x, vx1->vp[n1]->y, vx2->x, vx2->y, vx1->di, vx1->cn);
                print_pointer(vx1);
    ...
    if(vx2->di&(1<<n2)) {
        ...
        if(vx2->x == 525 && vx2->y == 657){
            printf("finish_two_dirs1: %p n2 = %d x1 = %d y1 = %d  vx2->vp[n1]->x = %d vx2->vp[n1]->y = %d vx1->x = %d vx1b->y = %d di = %d cn = %d\n",
                   vx2->vp[n2], n2, vx2->x, vx2->y, vx2->vp[n2]->x, vx2->vp[n2]->y, vx1->x, vx1->y, vx2->di, vx2->cn);
            print_pointer(vx2);
        }
...
#ifdef DEBUG
            if(vx2->di&(1<<nt)) {
                printf("finish_two_dirs1: Problem  %p n2 = %d x1 = %d y1 = %d  vx2->vp[n1]->x = %d vx2->vp[n1]->y = %d vx1->x = %d vx1b->y = %d di = %d cn = %d\n",
                       vx2->vp[n2], n2, vx2->x, vx2->y, vx2->vp[n2]->x, vx2->vp[n2]->y, vx1->x, vx1->y, vx2->di, vx2->cn);
                print_pointer(vx2);
    ...
    vx1->n++;
    if(!(vx1->di&(1<<n1))) vx1->di |= (1<<n1);
    vx1->vp[n1] = vx2;
    vx1->yx[n1] = yx1;

    vx2->n++;
    if(!(vx2->di&(1<<n2))) vx2->di |= (1<<n2);
    vx2->vp[n2] = vx1;
    vx2->yx[n2] = yx2;
    /*
    if(vx1->x == 525 && vx1->y == 657){
...
static inline uint32 finish_two_dirs2(Vertex *vx1, Vertex *vx2, uint32 yx1, uint32 yx2, int *dr, uint32 w)
{
    uint8 n1 = 8, n2 = 8, nt, i, d;
    int dyx, dyx1, dyx2, dx, dy, yxt, lm1, lm2;
...
static inline uint32 lines_approximation(Line_buff *buf, uint32 npix, uint32 w)
{
    ...
    if(i < 2){
        ...
        //if(buf[0].x + buf[0].y*w != buf[0].yx) printf("lines_approximation: first position changes less < 4 j = %d n = %d \n", j++, npix);
        //if(buf[i].x + buf[i].y*w != buf[i].yx) printf("lines_approximation: last  position changes less < 4 j = %d n = %d \n", j  , npix);
        ...
        while(1){
            if(j == i){
                ...
                if(!dl) {
                    //if(buf[0].x + buf[0].y*w != buf[0].yx) printf("lines_approximation: first position changes \n");
                    //if(buf[i].x + buf[i].y*w != buf[i].yx) printf("lines_approximation: last  position changes \n");
                    return 1;
                }
                j = 0; dl = 0;
            }
...
static inline uint32 lines_approximation1(uint8 *con, Line_buff *buf, uint32 npix, uint32 w, uint32 th)
{
    uint32 i = npix-1, j = 0, j1, dl, mid, len, lx, ly, yx, ln, cl, lf, rt;
    buf[0].np = i;
    ...
    while(1){
        ...
        if(!buf[j].in){
            ...
            if(ln == len) {
                cl = check_line3(buf, j, j1, &lf, &rt, w);
                if(j  == 0 && !lf) {
                    cl = 0;
                }
                if(j1 == i && !rt) {
    ...
    //The second pass
    dl = 0;
    j = buf[0].np; j1 = buf[buf[j].np].np;
    while(!(j == i || j1 == i || buf[j].np == i)){
        ...
        if(ln == len && check_line3(buf, j, j1, &lf, &rt, w) > th) {
            buf[j].np = j1;
            j1 = buf[j1].np;
            dl++;
        }
        else {
            if(dl) { j = j1; j1 = buf[buf[j1].np].np; dl = 0;}
            else   { j = buf[j].np; j1 = buf[j1].np; }
        }
    }
    ...
    for(y=1; y < h1; y++){
        ...
        for(x=1; x < w1; x++){
            ...
            if(grad[yx] > 253) {
                ...
                //x = 1449 y = 264  x = 1448 y = 264
                /*
                if((yx == 1449 + 264*w) || (yx == 1448 + 264*w)) {
                ...
                while(get_next_dir2(vxp, &dx, &dy)){
                    ...
                    //printf("yx1 = %d x = %d y = %d\n", yx1,  buf[i-1].x, buf[i-1].y);
                    ...
                    //Check if the last vertex have more than one neighbour if not remove it
                    if(j > 1){
                        //lines_approximation(buf, i, w);
                        lines_approximation1(con, buf, i, w, 60);
                        j = 0; vx1 = vxp;
...
uint32 seg_vertex5(uint8 *grad, uint8 *con, Vertex *vx, Vertex **vp, Vertex **vpn, uint32 *yxn, Line_buff *buf, uint32 w, uint32 h)
{
    uint32 i, j, j1, y, x, x1, y1, x2, y2, yx, yx1, yx2,  yw, nd, nd1, nd2, yxd, h1 = h-1, w1 = w-1;
    uint32  vxc = 0, lnc = 0, pow, vc1, vc2;
    int d,  dx, dy;
        ...
        for(x=1; x < w1; x++){
            ...
            if(grad[yx] > 253) {
                ...
                while(get_next_dir2(vxp, &dx, &dy)){
                    ...
                    i = get_next_vertex1(con, grad, buf, dx, dy, x, y, w);
                    yx1 = (buf[i-1].x>>1) + (buf[i-1].y>>1)*w;

                    if(i < 3) printf("next pixel i = %d x = %d y = %d x = %d y = %d con = %d grad = %d\n", i, x, y, buf[i-1].x, buf[i-1].y, con[yx1], grad[yx1]);

                    //if((yx1 == 1308 + 884*w) || (yx1 == 1306 + 886*w)) {
                    ...
                    if(j > 1){
                        ...
                        lines_approximation(buf, i, w);
                        j = 0; vx1 = vxp;
                        yx1 = (buf[j].x>>1) + (buf[j].y>>1)*w;
                        //nd1 = nd; vc1 = vxc;
                        while(1){
                            j1 = buf[j].np;
                            yx2 = (buf[j1].x>>1) + (buf[j1].y>>1)*w;
                            //printf("yx2 = %d \n", yx2);
                            //The end of vertexes
                            if(j1 == i-1){
                                if(grad[yx2] == 255) {
                                    ...
                                    //vx2->n = set_dir(con, yx2, &vx2->di, w);
                                    vx2->n = get_num_dir(con[yx2]);
                                    new_vertex1(con, vx2, vp, &vpn[lnc], &yxn[lnc+=8], buf[j1].x>>1, buf[j1].y>>1, yx2, w);
                                    finish_two_dirs(vx1, vx2, &buf[j+1], &buf[j1-1]);
                                    //x = 1308 y = 884 x = 1306 y = 886
...
static inline uint8 get_clockwise_dir1(Vertex *vx, uint8 nd, uint8 *nd1)
...
{
    uint32 i, j, l, vc = 0, vc1 = 0, tmp;
    uint8 nd, nd1, nd2, ndc, nda[8], na, k, k1;
    Vertex *vx, *vx1;
            ...
            for(l=0; l < na; l++){
                ...
                if(abs((int)vx->x - (int)vx1->x) < 2 && abs((int)vx->y - (int)vx1->y) < 2){
                    if(vx1->n == 2){
                        ...
                        get_clockwise_dir1(vx, nd, &nd1);
                        get_counterclockwise_dir(vx, nd, &nd2);
...
uint32  seg_vertex_draw3(uint8 *img, Vertex *vx2, uint32 vxc, uint32 w, uint32 h, uint32 w1, uint32 h1)
{
...
#ifdef DEBUG
            //Check if vertex is in another line
            //if(get_cn_num(vx->cn) == 1) if(img[vx->x + vx->y*w]) printf("Vertex is in another line x = %d y = %d\n", vx->x, vx->y);
#endif
...
#ifdef DEBUG
            //Check if vertex is in another line
            //if(!vx1->cn) if(img[vx1->x + vx1->y*w]) printf("Vertex is in another line x = %d y = %d\n", vx1->x, vx1->y);
#endif
            finish_dir1(vx1, nd1);
            //finish_dir2(vx1, nd1);
...
*/
    ...
    printf("Numbers of drawing vertex = %d lines = %d \n", vc, lc);
    return vc;
}
...
uint32  seg_vertex_draw4(uint8 *img, Vertex *vx2, uint32 vxc, Vertex **vp2, uint8 *dir, uint32 w, uint32 h, uint32 w1, uint32 h1)
{
    ...
    for(i=0; i < vxc; i++){
        ...
        //x = 1306 y = x = 1308 y = 884
        //if(vx2[i].x == 1306 && vx2[i].y == 886){
        /*
        if(vx2[i].x == 1551 && vx2[i].y == 810 || vx2[i].x == 1554 && vx2[i].y == 805 ){
            img[vx2[i].y*w + vx2[i].x] = 255;
            img[vx2[i].y*w + vx2[i].x-w] = 255;
            img[vx2[i].y*w + vx2[i].x-1] = 255;
            img[vx2[i].y*w + vx2[i].x+w] = 255;
            img[vx2[i].y*w + vx2[i].x+1] = 255;
        ...
        while(vx2[i].n && get_dir2(&vx2[i], &nd)){
            ...
            //vp1 = vp[i];
            //printf("Sart: %4d x = %4d y = %4d di = %d cn = %d nd = %d n = %d\n", i, vx->x, vx->y, vx->di, vx->cn, nd, vx->n);
            do {
                //printf("x = %4d y = %4d di = %d cn = %d nd = %d n = %d\n", vx->x, vx->y, vx->di, vx->cn, nd, vx->n);
                ...
                nd1 = find_pointer1(vx1, vx);
                if(nd1 == 10) {
                    print_around(img, vx1[j].x + vx1[j].y*w, w);
                    print_around(img, vx[j].x + vx[j].y*w, w);

                    return 0;
                }
                get_clockwise_dir1(vx1, nd1, &nd);
            ...
            } while(!(vx1 == &vx2[i] && nd2 == nd)); //;while(vx != vp[i]);
...
uint32 seg_regions(uint8 *con, Vertex *vx2, uint32 vxc, Vertex **vp2, uint8 *dir, uint16 *xy,  uint32 *pc, uint32 w, uint32 get)
{
    uint32 i, j, rc = 0, fn, sum = 0;
    int sq,  sqr = 0;
    uint8  nd, nd1, nd2;
    Vertex *vx, *vx1;
    ...
    for(i=0; i < vxc; i++){
        while(vx2[i].n && get_dir2(&vx2[i], &nd)){
            ...
            do {
                ...
                //x = 525 y = 657
                /*
                if(vx->x == 525 && vx->y == 657 ){
                    printf("x = %4d y = %4d di = %d cn = %d nd = %d n = %d\n", vx->x, vx->y, vx->di, vx->cn, nd, vx->n);
                    print_pointer(vx);
                }*/
                vx1 = vx->vp[nd];
                //if(i<3) printf("n = %d x = %d y = %d\n", vx1->n, vx1->x, vx1->y);

                nd1 = find_pointer1(vx1, vx);
#ifdef DEBUG
                if(nd1 == 10) {
                    print_con_grad(con, con, vx1->x + vx1->y*w, w);
                    print_con_grad(con, con, vx->x + vx->y*w, w);
                    //print_around(con, vx1->x + vx1->y*w, w);
                ...
                sq += (vx1->y + vx->y)*(vx1->x - vx->x);
                vx = vx1;
            } while(!(vx1 == &vx2[i] && nd2 == nd));

            if(sq > 0 ){
                vp2[rc] = &vx2[i]; dir[rc++] = nd2;
                //if(i<100) printf("Start n = %d x = %d y = %d sum = %d\n", vx2[i].n, vx2[i].x, vx2[i].y, fn);
    ...
    for(j=0; j < vxc; j++) {
        sum += vx2[j].n; //printf("%d %3d ", vx[j].n, vx[j].di);
        if(vx2[j].di != vx2[j].cn) {
            printf("seg_regions: %d  x = %d y = %d %o %o %o n = %d\n", j, vx2[j].x,  vx2[j].y, vx2[j].di, vx2[j].cn, vx2[j].cn^vx2[j].di, vx2[j].n);
            //printf("%d n = %d %o %o %o\n", j, vp[j]->n, vp[j]->di, vp[j]->cn, vp[j]->cn^vp[j]->di);
            //con[vx[j].y*w + vx[j].x-w] = 255;
            //con[vx[j].y*w + vx[j].x-1] = 255;
            //con[vx[j].y*w + vx[j].x+w] = 255;
...
#endif
    printf("Numbers of regions = %d number of pixels  = %d sum = %d\n", rc,  (*pc), sum);
    return rc;
}
...
uint32 seg_regions1(uint8 *con, Vertex *vx2, uint32 vxc, Vertex **vp2, uint8 *dir, uint32 w, uint32 h)
{
    ...
    for(j=0; j < vxc; j++) {
        if(vx[j].di != vx[j].cn) {
            printf("seg_regions1: %d  x = %d y = %d %o %o %o n = %d\n", j, vx[j].x,  vx[j].y, vx[j].di, vx[j].cn, vx[j].cn^vx[j].di, vx[j].n);
            //printf("%d n = %d %o %o %o\n", j, vp[j]->n, vp[j]->di, vp[j]->cn, vp[j]->cn^vp[j]->di);
            //con[vx[j].y*w + vx[j].x-w] = 255;
            //con[vx[j].y*w + vx[j].x-1] = 255;
            //con[vx[j].y*w + vx[j].x+w] = 255;
...
{
    uint32 i = 0, x, y, yx, yx1, yx2, tmp =0, cn = 0, lnc = 0, lc = 0,vxc = 0, yxa[2];
    int out = 0;
    Vector v;
    int dr[8] = { -1, -1-w, -w, +1-w, 1, 1+w, w, -1+w };
    xy = &xy[2];
    ...
    while(1){
        ...
        while(1){
            ...
            i += 2;
            //printf("%d npc = %d xb = %d yb = %d x = %d y = %d x1 = %d y1 = %d\n", i, npc, x, y, xy[i-2], xy[i-1], xy[i], xy[i+1]);
            v.x1 = xy[i-2]; v.y1 = xy[i-1];
            v.x2 = xy[i  ]; v.y2 = xy[i+1];
...
#ifdef DEBUG
            //Check if vertex is in another line
            if(!vp[yx1]) if(img[yx1]) {
                printf("seg_restore_vertex: Vertex is in another line x = %d y = %d\n", yx1%w, yx1/w);
                img[yx1] = 255;
                //img[yx1+1] = 255;
                //img[yx1-1] = 255;
                //img[yx1+w] = 255;
            ...
            if(!vp[yx1]) new_vertex2(&vx[vxc++], vp, &vpn[lnc], &yxn[lnc+=8], v.x1, v.y1, yx1, w);
            if(!vp[yx2]) {
                new_vertex2(&vx[vxc++], vp, &vpn[lnc], &yxn[lnc+=8], v.x2, v.y2, yx2, w);
                //yx = finish_two_dirs1(vp[yx1], vp[yx2], yxa[0], yxa[1], dr, w);
                yx = finish_two_dirs2(vp[yx1], vp[yx2], yxa[0], yxa[1], dr, w);
                if(yx) {
                    img[yx] =  255;
                    //img[yx+1] =  255;
                    //img[yx-1] =  255;
    ...
    printf("Numbers of pixels = %d %d vertex = %d lines = %d\n", npc, i, vxc, lc);
    return vxc;
...
static inline uint32 check_remove(Vertex *vx, uint8 nd)
{
	...
	vxt = vx; ndt = nd;
	do{
        get_clockwise_dir1(vxt, ndt, &ndt);
		vxt1 = vxt->vp[ndt];
	...
	vxt = vx; ndt = nd;
	do{
        get_counterclockwise_dir(vxt, ndt, &ndt);
		vxt1 = vxt->vp[ndt];
		ndt =  find_pointer1(vxt1, vxt);
		vxt = vxt1;
...
//vp1 temporary array for store vertexes
{
                   ...
                   vx = vpx; sq = 0; cn = 0; k = 0; nd3 = nd;
                    ...
                    while(1){
                        ...
                        if(nd1 == 10) {
                            print_con_grad(con, con, vx1->x + vx1->y*w, w);
                            print_con_grad(con, con, vx->x + vx->y*w, w);
                            print_con_grad(con, con, vx->x + (vx->y-2)*w, w);
                            return 0;
                        }
                        get_clockwise_dir1(vx1, nd1, &nd);
                        fd = finish_dir1(vx1, nd);
    ...
    if(sqr) printf("seg_remove_loops1: Toatl sq = %d\n", sqr);
    //printf("Toatl sq = %d\n", sqr);
...
uint32  seg_get_or_fill_color(uint8 *img, uint8 *con, uint8 *col, uint32 *buff, Vertex **vp, uint8 *dir,
                              uint32 rgc, uint32 w, uint32 h, uint32 w1, uint32 h1, uint32 get)
{
    uint32 i, j, k, yx, yxw, yxw1, vc1, pn=1, pn1, fd, cn, nx, bg;
    Vector v;
    ...
    //Start main cycle
    for(i=0; i < rgc; i++){
        vc = 0; vc1 = 0; sq = 0; yxw1 = 0; nx = 0; bg = 0;
        vx = vp[i]; nd = dir[i]; nd2 = nd;
        if(!get) cl = col[i];
        //printf("New region\n");
        ...
        do {
            ...
            //printf("nd1 = %d nd2 = %d \nx  = %d y  = %d \nx1 = %d y1 = %d \nx2 = %d y2 = %d \n",
            //       nd1, nd, vx1->x,vx1->y, vx1->yx[nd1]%w, vx1->yx[nd1]/w, vx1->yx[nd]%w, vx1->yx[nd]/w);

            //Check to remove sharp corners
            if(vx1->yx[nd1] == vx1->yx[nd]){
                if(!vc) bg = 1;
                else vc--;
                nx = 1;
            } else {
                if(!nx){
                    //printf("yxw = %d vc = %d\n", yxw, vc);
                    yxw = get_first_pixel2(con, vx1->x + vx1->y*w, vx1->yx[nd1], vx1->yx[nd], dr);
                    if(yxw) l1[vc++] = yxw;
                }
                nx = 0;
            }
...
uint32  seg_get_or_fill_color2(uint8 *img, uint8 *con, uint8 *col, uint32 *buff, Vertex **vp, uint8 *dir,
                              uint32 rgc, uint32 w, uint32 h, uint32 w1, uint32 h1, uint32 get)
{
    uint32 i, j, k, yx, yxw, yxw1, vc1, pn=1, pn1, fd, cn, nx, bg, test=0;
    Vector v;
    uint8  nd, nd1, nd2, ndn, sh = 15, cl = 200, dc;
    Vertex *vx, *vx1;
    int dr[8] = { -1, -1-w, -w, +1-w, 1, 1+w, w, -1+w };
    uint32 kx = ((w-2)<<sh)/(w1-3);
    uint32 ky = ((h-2)<<sh)/(h1-3);
    ...
    for(i=0; i < rgc; i++){
        ...
        do {
            ...
            sq += (vx1->y + vx->y)*(vx1->x - vx->x);
            nd1 =  find_pointer1(vx1, vx);
            dc = get_clockwise_dir1(vx1, nd1, &nd);

            if(!((abs(vx->x - vx1->x) == 1 && abs(vx->y - vx1->y) == 0) ||
                 (abs(vx->x - vx1->x) == 0 && abs(vx->y - vx1->y) == 1)) ){

                ndn = (nd1 == 7) ? 0 : nd1 + 1;

                if(nd != ndn && dc < 5){
                    yxw = vx1->x + vx1->y*w + dr[ndn];
                    if(!con[yxw]) {
                        l1[vc++] = yxw;
                        //con[yxw] = 255;
                        //if(yxw == 1009 + 1238*w) test =1;
                    }
                    //con[vx1->x + vx1->y*w + dr[ndn]] = 255;
                }
            }
            //printf("vp[i] = %p vx = %p x = %d y = %d di = %d cn = %d\n", vp[i], vx1, vx1->x, vx1->y, vx1->di, vx1->cn);
        ...
        /*
        if(test == 1){
            printf("vc = %d x1 = %d y1 = %d x2 = %d y2 = %d\n", vc, vx1->x, vx1->y, vx->x, vx->y);
            for(j=0; j < vc; j++) con[l1[j]] = 150;
            return 0;
        }
        if( vp[i]->x == 1009 && vp[i]->y == 1237) return 0;
        */
