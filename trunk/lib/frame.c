...
uint32 frame_segmetation(GOP *g, uint32 fn, WaletConfig *wc)
{
	...
	for(i=0; i < 1; i++) {
		...
		seg_vertex_draw3(f->dc[i].pic, f->vp, (uint32*)g->buf, vxc, f->dc[i].w,f->dc[i].h, f->dc[i].w, f->dc[i].h);
		...
		*/
		//j = i-1;
		//seg_vertex_draw3(f->dc[j].pic, f->vp, (uint32*)g->buf, vxc, f->dc[j].w, f->dc[j].h, f->dc[i].w, f->dc[i].h);
