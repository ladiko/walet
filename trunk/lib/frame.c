...
uint32 frame_segmetation(GOP *g, uint32 fn, WaletConfig *wc)
{
	...
	for(i=3; i < 4; i++) {
		...
		seg_vertex_draw4(f->dw[i].pic, f->dc[i].pic, g->buf, (uint32*)&g->buf[f->dg[i].w*f->dg[i].h>>1], f->vp, vxc, f->dg[i].w, f->dg[i].h, f->dg[i].w, f->dg[i].h);
		/*
		memset(f->dc[i].pic, 0, f->dg[i].w*f->dg[i].h);
		...
		*/
		j = i-1;
		rgc = seg_vertex_draw3(f->dc[j].pic, f->vp, (uint32*)g->buf, vxc, f->dc[j].w, f->dc[j].h, f->dc[i].w, f->dc[i].h);
