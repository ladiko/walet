...
/*Fix it optimization*/
	if(in1 != -1  ){
		if(img[yx  -w] != 255) if(img[yx  -w] > max) { max = img[yx  -w]; in =   -w; }
		if(img[yx+1-w] != 255) if(img[yx+1-w] > max) { max = img[yx+1-w]; in = +1-w; }
		if(img[yx+1  ] != 255) if(img[yx+1  ] > max) { max = img[yx+1  ]; in = +1  ; }
		if(img[yx+1+w] != 255) if(img[yx+1+w] > max) { max = img[yx+1+w]; in = +1+w; }
		if(img[yx  +w] != 255) if(img[yx  +w] > max) { max = img[yx  +w]; in =   +w; }
		goto end;
	}
