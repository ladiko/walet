...
uint32 frame_segmetation(GOP *g, uint32 fn, WaletConfig *wc)
{
	...
	for(i=3; i < 4; i++) {
		...
		memset(f->rg[i].pic, 0, f->rg[i].w*f->rg[i].h*sizeof(uint32));
		...
		seg_vertex_draw3(f->rg[i].pic, f->vp, vxc, f->dc[i].w,f->dc[i].h, f->dc[i].w, f->dc[i].h);
		//seg_get_one_color1(f->dw[i].pic,  f->rg[i].pic, g->buf, (uint32*)&g->buf[f->dg[i].w*f->dg[i].h>>2], f->dg[i].w, f->dg[i].h);
		/*
		memset(f->dc[i].pic, 0, f->dg[i].w*f->dg[i].h);
