...
uint32 frame_segmetation(GOP *g, uint32 fn, WaletConfig *wc)
{
    ...
    if(wc->ccol == CS420){
        ...
        seg_find_intersect6(f->dg[0].pic, f->dc[0].pic, (uint32*)g->buf, lmaxc, f->y[1].w, f->y[1].h);
       ...
       rgc = seg_remove_loops1(f->vx, &f->vpt[wc->w*wc->h>>2], f->vpt, f->ln, f->dm[0].pic, vxc, f->y[1].w, f->y[1].h);

        //seg_vertex_draw3(f->y1[1].pic, f->vp,  vxc, f->y[1].w, f->y[1].h, f->y[1].w, f->y[1].h);
      seg_vertex_draw4(f->y1[1].pic, f->vx,  vxc, f->y[1].w, f->y[1].h, f->y[1].w, f->y[1].h);
