...
void on_check_button_clicked(GtkObject *object, GtkWalet *gw)
{
	...
	utils_color_draw(frm1->line.pic , gdk_pixbuf_get_pixels(gw->orig[1]->pxb), w, h, 0);
...
void on_next_button_clicked(GtkObject *object, GtkWalet *gw)
{
	...
	if(nf){
		for(i=0; i<= nf; i++) {
			...
			utils_grey_draw(gw->gop->frames[i].line.pic, gdk_pixbuf_get_pixels(gw->orig[i*2+1]->pxb), w, h);
		...
		new_buffer (gw->orig[3], w, h);
		utils_grey_draw(gw->gop->frames[0].line.pic, gdk_pixbuf_get_pixels(gw->orig[3]->pxb), w, h);
		gtk_widget_queue_draw(gw->drawingarea[3]);

		new_buffer (gw->orig[0], w, h);
		utils_grey_draw(gw->gop->frames[0].edge.pic, gdk_pixbuf_get_pixels(gw->orig[0]->pxb), w, h);
		gtk_widget_queue_draw(gw->drawingarea[0]);
